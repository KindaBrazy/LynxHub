import {ReactElement} from 'react';

import {SvgProps} from '../SvgIconsContainer';

/* eslint max-len: 0 */

export function History_Color_Icon(props: SvgProps): ReactElement {
  return (
    <svg {...props} height="1em" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
      <g fill="none">
        <path
          fill={`url(#${props.id}_fluentColorHistory160)`}
          d="M7.698 5a.75.75 0 0 1 .75.75V7.5h1.25a.75.75 0 0 1 0 1.5h-2a.75.75 0 0 1-.75-.75v-2.5a.75.75 0 0 1 .75-.75"
        />
        <path
          fill={`url(#${props.id}fluentColorHistory161)`}
          d="M7.947 3.5a4.5 4.5 0 1 1-4.454 5.14a.75.75 0 1 0-1.485.212a6.001 6.001 0 1 0 1.94-5.324V2.75a.75.75 0 1 0-1.5 0v3c0 .414.335.75.75.75h2.5a.75.75 0 1 0 0-1.5H4.592a4.5 4.5 0 0 1 3.354-1.5"
        />
        <defs>
          <linearGradient
            x1="6.357"
            y2="8.988"
            x2="14.586"
            y1="12.633"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorHistory160`}>
            <stop stopColor="#d373fc" />
            <stop offset="1" stopColor="#6d37cd" />
          </linearGradient>
          <linearGradient
            x1="2"
            x2="5.234"
            y1="2.706"
            y2="16.186"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}fluentColorHistory161`}>
            <stop stopColor="#0fafff" />
            <stop offset="1" stopColor="#0067bf" />
          </linearGradient>
        </defs>
      </g>
    </svg>
  );
}
export function Pin_Color_Icon(props: SvgProps): ReactElement {
  return (
    <svg {...props} height="1em" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
      <g fill="none">
        <path
          fillRule="evenodd"
          clipRule="evenodd"
          fill={`url(#${props.id}_fluentColorPin160)`}
          d="m6.53 10.53l-3.25 3.25a.75.75 0 1 1-1.06-1.06l3.25-3.25z"
        />
        <path
          fill={`url(#${props.id}_fluentColorPin161)`}
          d="M10.059 2.445a1.5 1.5 0 0 0-2.386.353l-2.02 3.79l-2.811.938a.5.5 0 0 0-.196.828l5 5a.5.5 0 0 0 .828-.196l.937-2.811l3.779-2.023a1.5 1.5 0 0 0 .354-2.38z"
        />
        <path
          fillOpacity="0.8"
          fill={`url(#${props.id}_fluentColorPin162)`}
          d="M10.059 2.445a1.5 1.5 0 0 0-2.386.353l-2.02 3.79l-2.811.938a.5.5 0 0 0-.196.828l5 5a.5.5 0 0 0 .828-.196l.937-2.811l3.779-2.023a1.5 1.5 0 0 0 .354-2.38z"
        />
        <defs>
          <linearGradient
            x1="3.133"
            x2="8.986"
            y2="8.201"
            y1="12.867"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorPin160`}>
            <stop offset=".114" stopColor="#7b7bff" />
            <stop offset=".559" stopColor="#102784" />
          </linearGradient>
          <linearGradient
            x1="2.91"
            y1="4.159"
            x2="10.844"
            y2="12.392"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorPin161`}>
            <stop stopColor="#43e5ca" />
            <stop offset="1" stopColor="#1384b1" />
          </linearGradient>
          <radialGradient
            r="1"
            cx="0"
            cy="0"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorPin162`}
            gradientTransform="rotate(47.615 -7.043 18.496)scale(5.10742 12.8119)">
            <stop stopColor="#e362f8" />
            <stop offset="1" stopOpacity="0" stopColor="#96f" />
          </radialGradient>
        </defs>
      </g>
    </svg>
  );
}

export function Apps_Color_Icon(props: SvgProps): ReactElement {
  return (
    <svg {...props} height="1em" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
      <g fill="none">
        <path
          fill={`url(#${props.id}_fluentColorApps160)`}
          d="M2 4.5A1.5 1.5 0 0 1 3.5 3h3A1.5 1.5 0 0 1 8 4.5V8a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1z"
        />
        <path
          fill={`url(#${props.id}_fluentColorApps161)`}
          d="M11.5 8A1.5 1.5 0 0 1 13 9.5v3a1.5 1.5 0 0 1-1.5 1.5H8a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1z"
        />
        <path
          fill={`url(#${props.id}_fluentColorApps162)`}
          d="M8 13a1 1 0 0 1-1 1H3.5A1.5 1.5 0 0 1 2 12.5V9a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1z"
        />
        <path
          fill={`url(#${props.id}_fluentColorApps163)`}
          d="M9.69 2.442a1.5 1.5 0 0 1 2.122 0l1.752 1.752a1.5 1.5 0 0 1 0 2.121l-1.75 1.75a1.5 1.5 0 0 1-2.12 0L7.942 6.311a1.5 1.5 0 0 1 0-2.121z"
        />
        <defs>
          <linearGradient
            x1="2"
            x2="8"
            y1="3"
            y2="9"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorApps160`}>
            <stop stopColor="#b9c0c7" />
            <stop offset="1" stopColor="#889096" />
          </linearGradient>
          <linearGradient
            y2="8"
            x1="13"
            x2="7.429"
            y1="13.571"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorApps161`}>
            <stop stopColor="#63686e" />
            <stop offset="1" stopColor="#889096" />
          </linearGradient>
          <linearGradient
            x1="2"
            x2="8"
            y1="8"
            y2="11.857"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorApps162`}>
            <stop stopColor="#55595e" />
            <stop offset="1" stopColor="#383b3d" />
          </linearGradient>
          <linearGradient
            y1="7.06"
            x2="9.178"
            y2="2.471"
            x1="12.898"
            gradientUnits="userSpaceOnUse"
            id={`${props.id}_fluentColorApps163`}>
            <stop stopColor="#2764e7" />
            <stop offset="1" stopColor="#36dff1" />
          </linearGradient>
        </defs>
      </g>
    </svg>
  );
}

export function Youtube_Color_Icon(props: SvgProps): ReactElement {
  return (
    <svg {...props} height="1em" width="1.43em" viewBox="0 0 256 180" xmlns="http://www.w3.org/2000/svg">
      <path
        fill="#f00"
        d="M250.346 28.075A32.18 32.18 0 0 0 227.69 5.418C207.824 0 127.87 0 127.87 0S47.912.164 28.046 5.582A32.18 32.18 0 0 0 5.39 28.24c-6.009 35.298-8.34 89.084.165 122.97a32.18 32.18 0 0 0 22.656 22.657c19.866 5.418 99.822 5.418 99.822 5.418s79.955 0 99.82-5.418a32.18 32.18 0 0 0 22.657-22.657c6.338-35.348 8.291-89.1-.164-123.134"
      />
      <path fill="#fff" d="m102.421 128.06l66.328-38.418l-66.328-38.418z" />
    </svg>
  );
}
