...
import {SDArgSetting, SDLaunchConfig, SDSettingType} from '../../../AppState/InterfaceAndTypes';
...
// Import components
import FilterLaunchSettings from './FilterLaunchSettings/FilterLaunchSettings';
...
export default function WebUiLaunchSettings({repoUserName, ToggleSettings}: Props) {
  ...
  const SaveSettings = () => {
    ...
    ipcUserData.saveSDArgsToBat(dataToSave, repoUserName);
    ToggleSettings();
  };
  ...
  function fetchComponentBySettingType(
    ...
    switch (setting.Type) {
      ...
      case SDSettingType.ChooseDirectory: {
        const element: React.JSX.Element = (
          <OpenDialog
  ...
  useEffect(() => {
    ...
    launchArgs.env.forEach((envVar: {id: string; value: string}): void => {
      if (envVar.id !== 'COMMANDLINE_ARGS') {
        if (environmentVariables[envVar.id].Type === SDSettingType.CheckBox && envVar.value === '') {
    ...
    launchArgs.cl.forEach((clArg: {id: string; value: string}): void => {
      ['Configuration', 'Performance', 'Features'].some((key: string) => {
        ...
        if (foundKey) {
          if (sda1CommandLines[key][foundKey].Type === SDSettingType.CheckBox && clArg.value === '') {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showListMenu && <FilterLaunchSettings launchArgs={launchArgs} launchArgsDispatch={launchArgsDispatch} setShowListMenu={setShowListMenu} />}
      </AnimatePresence>
    </>
  );
}
