    ...
    return (
      <motion.div
        ...
        <div className="flex items-start gap-3 group">
          <motion.div
            className={`w-2 h-2 rounded-full mt-1.5 shrink-0 transition-colors duration-200 ${depth > 0
        ...
        <div key={label}>
          ...
          <div className="space-y-1">
            {subItems.map((subItem, index) => (
              <ChangelogEntry item={subItem} depth={depth + 1} id={index} key={`${label}_${index}`} />
            ))}
          </div>
        </div>
      ))}
    ...
    <div>
      ...
      <div className="space-y-2">
        {entries.map((entry, index) => (
          <ChangelogEntry item={entry} depth={0} id={index} key={`${category}_${index}`} />
        ))}
      </div>
    </div>
  );
  ...
  return (
    <motion.div
      ...
      <div
        ...
        rounded-xl px-4 py-4 mb-6 transition-all duration-300 ${isLatest ? 'border-secondary-200 hover:border-secondary-300' : 'border-foreground-100 hover:border-foreground-200'
          }`}>
